namespace Lingu.CC
{
    ...
    public abstract class AbstractLinguVisitor<T> where T : class
    {
        ...
        protected abstract T OnRuleDrop(INonterminalToken token);
        protected abstract T OnRulePromote(INonterminalToken token);
        ...
        protected W Visit<W>(IToken token) where W : T
        {
            ...
            return token.Symbol.Id switch
            {
                ...
                LinguId.Angrammar => (W)OnAngrammar((INonterminalToken)token),
                ...
                LinguId.RuleDrop => (W)OnRuleDrop((INonterminalToken)token),
                LinguId.RulePromote => (W)OnRulePromote((INonterminalToken)token),
    ...
    public abstract class LinguVisitor<T> : AbstractLinguVisitor<T> where T : class
    {
        ...
        protected override T OnRuleDrop(INonterminalToken token)
        {
            Debug.Assert(token != null);
            return DefaultOn(token);
        }
        protected override T OnRulePromote(INonterminalToken token)
        {
            Debug.Assert(token != null);
            return DefaultOn(token);
        }
